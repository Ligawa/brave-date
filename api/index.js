...
async function profile(req, res) {
  ...
  if (req.method === 'GET') { const { data, error } = await supabase.from('dating_profiles').select('*').eq('id', user.id).single(); if (error) return json(res, 404, { message: 'Profile not found.' }); return json(res, 200, { user: profileShape(data) }) }
  const input = await body(req); const { data, error } = await supabase.from('dating_profiles').upsert({ id: user.id, email: user.email, first_name: input.first_name, last_name: input.last_name, passion: input.passion, phone_number: input.phone_number, birth_date: input.birth_date, gender: input.gender, bio: input.bio, updated_at: new Date().toISOString() }).select().single(); if (error) return json(res, 400, { message: error.message }); return json(res, 200, { user: profileShape(data), message: 'Profile updated.' })
}
async function users(req, res) { const user = await userFor(req); const { data, error } = await client(req).from('dating_profiles').select('*').neq('id', user.id).order('created_at', { ascending: false }); if (error) return json(res, 400, { message: error.message }); json(res, 200, { result: data.map(profileShape) }) }
async function matches(req, res) { const user = await userFor(req); const supabase = client(req); if (req.method === 'POST') { const input = await body(req); const { data: target } = await supabase.from('dating_profiles').select('id').eq('email', input.match).single(); if (!target) return json(res, 404, { message: 'User not found.' }); const { error } = await supabase.from('dating_matches').upsert([{ user_id: user.id, matched_user_id: target.id }, { user_id: target.id, matched_user_id: user.id }], { onConflict: 'user_id,matched_user_id' }); if (error) return json(res, 400, { message: error.message }); return json(res, 201, { message: 'Match created.' }) } const { data, error } = await supabase.from('dating_matches').select('matched_user_id, dating_profiles:matched_user_id(*)').eq('user_id', user.id); if (error) return json(res, 400, { message: error.message }); json(res, 200, { result: (data || []).map(x => profileShape(x.dating_profiles)) }) }
async function messages(req, res) { const user = await userFor(req); const supabase = client(req); if (req.method === 'POST') { const input = await body(req); const { data, error } = await supabase.from('dating_messages').insert({ sender_id: user.id, receiver_id: input.receiver_id, content: input.content, message_type: input.type || 'text', media_pathname: input.media_pathname }).select().single(); if (error) return json(res, 400, { message: error.message }); return json(res, 201, { result: data }) } const receiver = new URL(req.url, 'https://localhost').searchParams.get('receiver_id') || new URL(req.url, 'https://localhost').searchParams.get('receiver'); let query = supabase.from('dating_messages').select('*').or(`sender_id.eq.${user.id},receiver_id.eq.${user.id}`).order('created_at'); if (receiver) query = query.or(`and(sender_id.eq.${user.id},receiver_id.eq.${receiver}),and(sender_id.eq.${receiver},receiver_id.eq.${user.id})`); const { data, error } = await query; if (error) return json(res, 400, { message: error.message }); json(res, 200, { result: data || [] }) }
async function upload(req, res) { const user = await userFor(req); if (req.method !== 'POST') return json(res, 405, { message: 'Method not allowed.' }); const input = await body(req); const contentType = String(input.content_type || ''); const size = Number(input.size_bytes || 0); if (!/^image\/(jpeg|png|webp|gif)$/.test(contentType) || size <= 0 || size > 8 * 1024 * 1024) return json(res, 400, { message: 'Invalid image.' }); const pathname = `dating/${user.id}/${crypto.randomUUID()}`; const blob = await put(pathname, Buffer.from(input.base64, 'base64'), { access: 'public', contentType }); const { error } = await client(req).from('dating_profile_photos').insert({ user_id: user.id, pathname: blob.pathname, url: blob.url, content_type: contentType, size_bytes: size }); if (error) { await del(blob.url); return json(res, 400, { message: error.message }) } await client(req).from('dating_profiles').update({ profile_picture: blob.url }).eq('id', user.id); json(res, 200, { url: blob.url, pathname: blob.pathname, message: 'Profile image updated.' }) }
