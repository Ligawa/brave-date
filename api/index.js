...
async function matches(req, res) { const user = await userFor(req); const supabase = client(req); if (req.method === 'POST') { const input = await body(req); const { data: target } = await supabase.from('liebena_profiles').select('id').eq('email', input.match).single(); if (!target) return json(res, 404, { message: 'User not found.' }); const { error } = await supabase.from('liebena_matches').upsert([{ user_id: user.id, matched_user_id: target.id }, { user_id: target.id, matched_user_id: user.id }], { onConflict: 'user_id,matched_user_id' }); if (error) return json(res, 400, { message: error.message }); return json(res, 201, { message: 'Match created.' }) } const { data: matchRows, error: matchError } = await supabase.from('liebena_matches').select('matched_user_id').eq('user_id', user.id); if (matchError) return json(res, 400, { message: matchError.message }); const ids = (matchRows || []).map(row => row.matched_user_id); if (!ids.length) return json(res, 200, { result: [] }); const { data: profiles, error: profileError } = await supabase.from('liebena_profiles').select('*').in('id', ids); if (profileError) return json(res, 400, { message: profileError.message }); json(res, 200, { result: (profiles || []).map(profileShape) }) }
